...
function update_statistics(start, end) {
    ...
    $('.total_due').html(loader);
    $('.cashReceiveds').html(loader);
    $('.cashhand').html(loader);
    $('.total_discounts').html(loader);

    // $('.cashReceived').html(loader);
    // $('.cashhand').html(loader);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('.total_paid_purchasde_due').html(__currency_trans_from_en(data.PurchasepaidDue, true));
            $('.total_paid_sell_due').html(__currency_trans_from_en(data.sellPaidDue, true));
            $('.total_due').html(__currency_trans_from_en(data.totals_due, true));
            $('.cashReceiveds').html(__currency_trans_from_en(data.cashReceived, true));
            $('.cashhand').html(__currency_trans_from_en(data.cashhand, true));
            $('.total_discounts').html(__currency_trans_from_en(data.total_discounts, true));
