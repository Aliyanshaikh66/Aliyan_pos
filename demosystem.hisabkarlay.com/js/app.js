$(document).ready(function() {
    ...
    var currency_table = $('#currencyss_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/currencychanges',
        columnDefs: [{
            targets: 2,
            orderable: false,
            searchable: false,
    ...
    $(document).on('click', 'button.edit_currency_button', function() {
        $('div.currency_modal').load($(this).data('href'), function() {
            $(this).modal('show');
    ...
    $(document).on('change', '#has_no_sell_from, #cg_filter, #status_filter, #assigned_to , #cus_area', function() {
        contact_table.ajax.reload();
    });
